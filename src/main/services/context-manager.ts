...
import type { IContextManager, ChatMessage } from '../core/interfaces';
import { conversationStore } from '../store/conversation-store';

export class ContextManager implements IContextManager {
  constructor(private store: typeof conversationStore = conversationStore) {}
  getContext(): ContextWindow {
    const allTurns = this.store.getRecentTurns(30);
  ...
  addTurn(turn: Omit<ConversationTurn, 'id' | 'timestamp'>): ConversationTurn {
    return this.store.addTurn(turn);
  }
  ...
  buildMessages(context: ContextWindow, currentUserMessage: string): ChatMessage[] {
    ...
    for (const turn of context.recentTurns) {
      if (turn.role === 'user' || turn.role === 'assistant') {
        ...
        messages.push({ role: turn.role as ChatMessage['role'], content });
      }
    }
